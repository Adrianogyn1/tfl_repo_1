...
if (!DB_URL) {
    console.error('❌ ERRO CRÍTICO: DATABASE_URL não foi encontrada. Verifique o caminho do seu arquivo .env');
    process.exit(1);
}
...
sequelize.sync()
    .then(() => console.log('⚡ Banco PostgreSQL (Supabase) e tabelas sincronizados!'))
    .catch(err => console.error('❌ Erro ao sincronizar o banco PostgreSQL:', err));
...
function parseQueryStringToWhere(queryString) {
    ...
    while ((match = regex.exec(filterPart)) !== null) {
        ...
        const finalValue = (seqOp === Op.iLike || seqOp === Op.like) ? `%${value}%` : value;
