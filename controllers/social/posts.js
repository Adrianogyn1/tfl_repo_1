  ...
  try {
    ...
    const [postLikesCount, postCommentsCount] = await Promise.all([
      repo.Like.findAll({
        attributes: [
          "postId",
          [repo.sequelize.fn("COUNT", repo.sequelize.col("id")), "total"],
        ],
    ...
    const totalLikesMap = new Map(
      postLikesCount.map((l) => [String(l.postId), l.total]),
    );
    const totalCommentsMap = new Map(
      postCommentsCount.map((c) => [String(c.postId), c.total]),
    );
    ...
    const commentLikesCount =
      commentIds.length > 0
        ...
        : [];
    const totalCommentLikesMap = new Map(
      commentLikesCount.map((l) => [String(l.commentId), l.total]),
    );
    ...
    const allUserIds = [
      ...new Set([...authorIds, ...commentAuthorIds, ...likeUserIds]),
    ];
    ...
    const photoMap = new Map(
      photosModels.map((p) => [String(p.userId), p.url]),
    );
    ...
    return res
      .status(500)
      .json({ success: false, error: "Erro interno ao carregar o feed" });
  }
}
...
async function deletePost(req, res) {
  ...
  if (!post) {
    return res
      .status(404)
      .json({ error: "Postagem não encontrada.", success: false, data: {} });
  }

  if (!user || (post.userId || post.userid) != user.id) {
    console.log(
      `Usuário ${user?.id || req.userId} tentou apagar post de ${post?.userId || post?.userid}`,
    );
    return res
      .status(401)
      .json({ error: "Usuário não autorizado.", success: false, data: {} });
  }
  ...
  console.log(
    `Usuário ${user?.id || req.userId} apagou post de ${post?.userId || post?.userid}`,
  );
