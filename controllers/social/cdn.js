...
async function getSignature(req, res) {
  ...
  global.activeUploadTokens[uploadToken] = {
    used: false,
    expires: expireAt * 1000,
  };
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erro ao registrar informações do arquivo." });
  }
}
...
function handleCloudinaryWebhook(req, res) {
  ...
  if (
    !token ||
    !global.activeUploadTokens ||
    !global.activeUploadTokens[token]
  ) {
