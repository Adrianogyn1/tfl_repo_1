...
async function uploadDirectToCDN(req, res) {
  ...
  try {
    ...
    return res.status(500).json({
      error: "Falha no processamento ou envio do arquivo para a CDN.",
    });
  }
}
...
async function uploadLocal(req, res) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erro interno ao salvar arquivo localmente." });
  }
}
...
function sendFile(req, res, sendPacketToUserCallback, broadcastCallback) {
  ...
  if (!FileId)
    return res
      .status(400)
      .json({ error: "O identificador FileId é obrigatório." });
