...
async function deleteComment(req, res) {
  ...
  if ((comment.userId || comment.userid) != req.userId) {
    console.log(
      `Usuário ${req.userId} tentou apagar comentário de ${comment.userId || comment.userid}`,
    );
    return res
      .status(403)
      .json({ error: "Usuário não autorizado para apagar esse comentário." });
  }
...
async function likeComment(req, res) {
  ...
  if (!comment) {
    return res
      .status(404)
      .json({ error: "Comentário nao encontrado.", success: false, data: {} });
  }
...
async function dislikeComment(req, res) {
  ...
  await repo.Like.destroy({
    where: { commentId: comment.id, userId: req.userId },
  });
  return res.status(201).json({ success: true });
}
