...
function addFriend(req, res, sendPacketToUserCallback) {
  ...
  const fr = repo.Friend.where(
    (f) => f.userid === userId && f.targetid === targetId,
  )[0];
...
function getFriends(req, res) {
  ...
  const friends = repo.Friend.where(
    (f) => f.userid === id || (f.targetid === id && f.status === "Accepted"),
  );
...
function follow(req, res) {
  ...
  const fl = repo.Follow.where(
    (f) => f.userId === user.id && f.targetId === targetUserId,
  )[0];
...
function ignore(req, res) {
  ...
  const userId = req.userId;
  const ig = repo.Ignore.where(
    (i) => i.userId === userId && i.targetId === targetUserId,
  )[0];
