...
function checkData(outfitData) {
  let error = "";
  try {
    if (!outfitData) throw new Error("Dados do visual são obrigatórios.");

    if (!outfitData.name) throw new Error("O nome do visual é obrigatório.");
  } catch (e) {
    error = e.message;
  }
  return error;
...
async function getAll(req, res) {
  ...
  try {
    ...
    return res.json({
      ...
      limit: limit,
      nextPage:
        outfits?.count <= 0 ? 0 : Math.ceil((offset + limit) / limit) + 1,
    });
  } catch (err) {
    return res
      .status(500)
...
async function remove(req, res) {
  try {
    ...
    return res.json({
      success: true,
      message: "Outfit and its items deleted successfully",
    });
  } catch (err) {
    return res.status(500).json({ error: err.message, success: false });
  }
