...
async function getAll(req, res) {
  ...
  try {
    ...
    return res.json({
      ...
      limit: limit,
      nextPage:
        materials?.count <= 0 ? 0 : Math.ceil((offset + limit) / limit) + 1,
    });
  } catch (err) {
    return res
      .status(500)
...
async function getById(req, res) {
  try {
    ...
    if (!material) {
      return res
        .status(404)
        .json({ error: "Material not found", success: false });
    }
  ...
  try {
    ...
    if (req.files && req.files["texturas[]"]) {
      ...
      for (const file of texturasArray) {
        ...
        console.log(
          `textura ${file.originalname} hash: ${fileHash} ${texture.userId} ${userId} por favor verificar`,
        );
...
async function create(req, res) {
  try {
    if (!req.body || Object.keys(req.body).length === 0) {
      return res
        .status(400)
        .json({ error: "Data body is required", success: false });
    }
