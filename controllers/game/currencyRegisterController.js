...
function checkData(registerData) {
  ...
  try {
    ...
    if (!registerData.userId) throw new Error("O userId é obrigatório.");
...
async function getAll(req, res) {
  try {
    ...
    const limit =
      parseInt(req.query?.limit || req.body?.limit || req.body?.take) || 50;
    const offset =
      parseInt(req.query?.offset || req.body?.skip || req.body?.offset) || 0;
    ...
    registers.rows = registers.rows.filter(
      (register) => register.userId === req.userId,
    );
    //atualizar o count
...
async function getById(req, res) {
  try {
    ...
    if (!register) {
      return res
        .status(404)
        .json({ error: "Currency register not found", success: false });
    }
...
async function remove(req, res) {
  try {
    ...
    return res.json({
      success: true,
      message: "Currency register deleted successfully",
    });
  } catch (err) {
    return res.status(500).json({ error: err.message, success: false });
  }
...
async function UpdateCurrencyRegister(
  registerData,
  userId,
  transaction = null,
) {
...
async function CreateCurrencyRegister(
  ...
  try {
    ...
    if (!register || register.length === 0)
      throw new Error("Currency register not created");
