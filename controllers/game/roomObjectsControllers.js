...
function checkData(objectData) {
  let error = "";
  try {
    if (!objectData) throw new Error("Dados do objeto são obrigatórios.");

    if (!objectData.data) throw new Error("O campo data é obrigatório.");

    if (!objectData.ownnerId) throw new Error("O ownnerId é obrigatório.");

    if (!objectData.roomId) throw new Error("O roomId é obrigatório.");
  } catch (e) {
    error = e.message;
  }
  return error;
...
async function getById(req, res) {
  try {
    ...
    if (!object) {
      return res
        .status(404)
        .json({ error: "Object not found", success: false });
    }
...
async function createAll(req, res) {
  ...
  try {
    if (!Array.isArray(list) || list.length === 0) {
      return res
        .status(400)
        .json({ error: "Invalid or empty array list", success: false });
    }
