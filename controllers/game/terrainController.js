...
async function HandleTerrainUpdate(req) {
  ...
  try {
    ...
    const room = await repo.RoomModel.findOne({
      where: { uid: terrain.target_uid },
    });
    if (!room) throw new Error("Room not found");

    let terrainData = await repo.Terrain.findOne({
      where: { uid: terrain.uid },
    });
    ...
    const novoHash = crypto
      .createHash("sha256")
      .update(fileInput.buffer)
      .digest("hex");
    const precisaAtualizarArquivo =
      !terrainData ||
      terrainData.hash !== novoHash ||
      !fs.existsSync(caminhoFinal);
    ...
    await repo.sequelize.transaction(async (t) => {
      ...
      await terrainData.update(
        { path: caminhoFinal, hash: novoHash },
        { transaction: t },
      );
    });

    if (backupRealizado && fs.existsSync(caminhoBackup))
