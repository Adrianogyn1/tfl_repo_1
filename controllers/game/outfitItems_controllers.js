...
function checkData(itemData) {
  let error = "";
  try {
    if (!itemData) throw new Error("Dados do item do visual são obrigatórios.");

    if (!itemData.type) throw new Error("O tipo (type) do item é obrigatório.");

    if (!itemData.prefabId) throw new Error("O prefabId é obrigatório.");

    if (!itemData.outfitId) throw new Error("O outfitId é obrigatório.");
  } catch (e) {
    error = e.message;
  }
  return error;
...
async function getAll(req, res) {
  const outfitId =
    req.params?.outfitId || req.body?.outfitId || req.query?.outfitId || null;
...
async function getById(req, res) {
  try {
    ...
    if (!item) {
      return res
        .status(404)
        .json({ error: "Outfit item not found", success: false });
    }
