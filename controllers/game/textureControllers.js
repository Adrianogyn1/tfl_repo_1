...
async function getTexture(req, res) {
  ...
  if (pathFile[0] == ".") pathFile = pathFile.substring(2);
...
function checkData(textureData) {
  let error = "";
  try {
    if (!textureData) throw new Error("Dados da textura são obrigatórios.");

    if (!textureData.name) throw new Error("O nome da textura é obrigatório.");

    if (!textureData.path) throw new Error("O path da textura é obrigatório.");
  } catch (e) {
    error = e.message;
  }
  return error;
...
async function getAll(req, res) {
  ...
  try {
    ...
    return res.json({
      ...
      limit: limit,
      nextPage:
        textures?.count <= 0 ? 0 : Math.ceil((offset + limit) / limit) + 1,
    });
  } catch (err) {
    return res
      .status(500)
...
async function getById(req, res) {
  try {
    ...
    if (!texture) {
      return res
        .status(404)
        .json({ error: "Texture not found", success: false });
    }
