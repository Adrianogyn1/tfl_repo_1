...
async function seedDatabase() {
  ...
  try {
    ...
    console.log(
      `🤖 Tratando ${data.users.length} usuários para o formato do sistema...`,
    );
...
async function popularTabelaSeVazia() {
  try {
    ...
    if (totalPerfis === 0) {
      console.log(
        `📥 Tabela 'MatchProfile' vazia. Buscando ${QUANTIDADE_REGISTROS} usuários do DummyJSON...`,
      );
      const usersRes = await axios.get(
        `https://dummyjson.com/users?limit=${QUANTIDADE_REGISTROS}`,
      );
    ...
    if (totalPosts === 0) {
      console.log(
        `📥 Tabela 'Post' vazia. Buscando ${QUANTIDADE_REGISTROS} posts do DummyJSON...`,
      );
      const postsRes = await axios.get(
        `https://dummyjson.com/posts?limit=${QUANTIDADE_REGISTROS}`,
      );
    ...
    if (totalComentarios === 0) {
      console.log(
        `📥 Tabela 'Comment' vazia. Buscando ${QUANTIDADE_REGISTROS} comentários do DummyJSON...`,
      );
      const commentsRes = await axios.get(
        `https://dummyjson.com/comments?limit=${QUANTIDADE_REGISTROS}`,
      );
...
async function iniciarSeed() {
  try {
    ...
    console.log(
      "🏁 Todo o banco de dados foi verificado e populado com sucesso!",
    );
    process.exit(0);
  } catch (error) {
