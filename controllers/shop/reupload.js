...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    console.error('Erro: SUPABASE_URL ou SUPABASE_SERVICE_ROLE_KEY não foram definidos no .env');
    process.exit(1);
}
        ...
        for (const container of containers) {
            try {
                if (container.thumb && !container.thumb.startsWith('http')) {
                    const fullPath = path.join(__dirname, '../../uploads/shop', container.thumb);
                    ...
                    for (const item of container.items) {
                        ...
                        for (const key in textures) {
                            ...
                            if (currentPath && !currentPath.startsWith('http')) {
                                const fullPath = path.join(__dirname, '../../uploads/shop', currentPath);
                ...
                console.error(`[ERRO] Falha no Container ID ${container.id}:`, err.message);
            }
        }
