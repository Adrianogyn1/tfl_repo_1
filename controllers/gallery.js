...
async function addPhotoProfile(req, res) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erro interno ao adicionar foto de perfil." });
  }
}
...
async function deletePhoto(req, res, broadcastCallback) {
  ...
  try {
    ...
    if (photo.userId !== req.userId) {
      return res
        .status(403)
        .json({ error: "Não autorizado a deletar esta foto." });
    }

    if (photo.url) {
      const armazenadoNaCDN =
        photo.isCDN || photo.url.includes(".cloudinary.com");
...
async function addComment(req, res) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erro interno ao adicionar comentário." });
  }
}
...
async function deleteComment(req, res) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erro interno ao deletar comentário." });
  }
}
