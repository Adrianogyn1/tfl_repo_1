...
function checkData(serverData) {
  ...
  try {
    ...
    if (!serverData.scene) throw new Error("scene is required");
...
async function getAll(req, res) {
  ...
  try {
    const servers = await repo.ServerModel.findAndCountAll({
      limit: limit,
      offset: offset,
    });
...
async function getById(req, res) {
  try {
    ...
    if (!server) {
      return res
        .status(404)
        .json({ error: "Server not found", success: false });
    }
...
async function create(req, res) {
  try {
    ...
    const server = await repo.ServerModel.create(serverData);
    return res.json({ success: true, data: server });
  } catch (err) {
    return res.status(500).json({ error: err.message, success: false });
  }
...
async function update(req, res) {
  try {
    ...
    const serverExists = await repo.ServerModel.findByPk(req.params.id);
    if (!serverExists) {
      return res
        .status(404)
        .json({ error: "Server not found", success: false });
    ...
    await repo.ServerModel.update(req.body, { where: { id: req.params.id } });
    const updatedServer = await repo.ServerModel.findByPk(req.params.id);
...
async function remove(req, res) {
  try {
    ...
    await repo.ServerModel.destroy({ where: { id: req.params.id } });
