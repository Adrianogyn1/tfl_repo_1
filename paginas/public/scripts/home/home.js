$(document).ready(function () {
  ...
  $("#btnSignIn").click(async function () {
    ...
    if (res && res.token) {
      ...
      if (profile) {
        $("#myUserAvatar").text(
          String(profile.userName || profile.username || "U")
            .substring(0, 1)
            .toUpperCase(),
        );
      }
  ...
  async function openProfilePopup(userId, userName) {
    ...
    $("#btnModalLikeProfile")
      .removeClass("bg-pink-800")
      .addClass("bg-pink-600")
      .data("liked", "false")
      .find("span")
      .text("Curtir Perfil");
    ...
    if (profileData && profileData.data) {
      ...
      if (user.likedProfile || user.liked === true) {
        $("#btnModalLikeProfile")
          .removeClass("bg-pink-600")
          .addClass("bg-pink-800")
          .data("liked", "true")
          .find("span")
          .text("Remover Curtida");
      }
    }
  }

  function renderModalFeed() {
    const userPosts = cachedPosts.filter(
      (p) => p.userid == activeProfileId || p.userId == activeProfileId,
    );
    ...
    if (userPosts.length === 0) {
      $modalTarget.html(
        '<p class="text-gray-500 text-center py-8 text-sm">Este usuário não possui publicações.</p>',
      );
    } else {
  ...
  // --- Rotas do Perfil ---
  $("#btnModalFollow").click(function () {
    if (activeProfileId)
      apiCall($("#epFollow").val(), "POST", { targetId: activeProfileId });
  });
  $("#btnModalUnfollow").click(function () {
    if (activeProfileId)
      apiCall($("#epUnfollow").val(), "POST", { targetId: activeProfileId });
  });
  ...
  $("#btnModalLikeProfile").click(async function () {
    ...
    const res = await apiCall($("#epLikeProfile").val(), method, {
  ...
  $("#btnModalAddFriend").click(function () {
    if (activeProfileId)
      apiCall($("#epAddFriend").val(), "POST", { targetId: activeProfileId });
  });
  $("#btnModalRemoveFriend").click(function () {
    if (activeProfileId)
      apiCall($("#epRemoveFriend").val(), "POST", {
        targetId: activeProfileId,
      });
  });
  $("#btnModalMatchIn").click(function () {
    if (activeProfileId)
      apiCall($("#epMatchIn").val(), "POST", { targetId: activeProfileId });
  });
  $("#btnModalMatchOut").click(function () {
    if (activeProfileId)
      apiCall($("#epMatchOut").val(), "POST", { targetId: activeProfileId });
  });

  $("#btnModalSendMsg").click(function () {
    const msgText = $("#modalMsgText").val();
    if (activeProfileId && msgText) {
      apiCall($("#epSendMsg").val(), "POST", {
        targetId: activeProfileId,
        text: msgText,
      }).then((res) => {
  ...
  $(document).on("click", ".btn-like-post", async function () {
    ...
    if (res) {
      if (isLiked) {
        $(this)
          .attr("data-liked", "false")
          .removeClass("text-pink-500")
          .addClass("text-gray-400")
          .html('<i class="fa-solid fa-heart"></i> Curtir Post');
      } else {
        $(this)
          .attr("data-liked", "true")
          .removeClass("text-gray-400")
          .addClass("text-pink-500")
          .html('<i class="fa-solid fa-heart"></i> Remover Curtida');
      }
    }
  });
  ...
  function renderPostCard(post) {
    ...
    let commentsHtml =
      '<p class="text-gray-500 text-[11px] italic pl-1">Sem comentários.</p>';
  ...
  async function loadFeed() {
    ...
    if (cachedPosts.length === 0) {
      $feedTarget.html(
        '<p class="text-gray-500 text-center py-8 text-sm">Nenhuma publicação por aqui.</p>',
      );
      return;
    }
    ...
    cachedPosts.forEach((post) => {
      $feedTarget.append(renderPostCard(post));
    });
  }
  ...
  $(document).on("click", ".btn-submit-comment", async function () {
    ...
    const data = await apiCall($("#epComment").val(), "POST", {
      postId,
      text: commentText,
    });
    if (data) {
  ...
  $(document).on("click", ".open-profile-btn", function (e) {
    ...
    if (targetId && targetId !== "Sistema")
      openProfilePopup(targetId, targetName);
  });
  ...
  async function openModal() {
    ...
    try {
      ...
      userList.innerHTML =
        '<p class="text-red-500 text-center py-4">Erro ao carregar.</p>';
    }
  }
