...
$(document).ready(function () {
    ...
    $(document).on('click', '.model-card', function () {
        $('.model-card').removeClass('border-blue-500 bg-blue-900/40').addClass('border-gray-700');
        $(this).removeClass('border-gray-700').addClass('border-blue-500 bg-blue-900/40');
...
window.addEventListener('change', function (e) {
    ...
    if (input && input.type === 'file') {
        ...
        console.log(`[FILE-DEBUG] Input alterado! Index: ${index}, Key: ${key}, Arquivos:`, files);

        if (index === undefined || !containerItems[index]) {
            console.error(`[FILE-ERROR] Item no índice ${index} não existe na memória!`);
            return;
        }

        if (files.length > 0) {
            if (!containerItems[index].uploadFiles) containerItems[index].uploadFiles = {};
            containerItems[index].uploadFiles[key] = files;

            const fileNames = files.map(f => f.name).join(', ');
            containerItems[index].textures_paths[key] = fileNames;

            console.log(`[FILE-SUCCESS] Arquivos salvos na memória do item [${index}]:`, containerItems[index].uploadFiles);

            const labelText = files.length === 1 ? files[0].name : `${files.length} arquivos selecionados`;
...
}, true); // 'true' força a execução na fase de captura

    // Clique na Sanfona (Abrir/Fechar)
    $(document).on('click', '#container-items-list .accordion-header', function (e) {
    ...
    $(document).on('click', '#container-items-list .btn-add-prop', function () {
        ...
        console.log(`[SHOP-PROP] Adicionando [${type}] chave [${key}] no item [${index}]`);

        if (!key || !containerItems[index]) return;

        const defaults = { values: 0.0, colors: '#ffffff', ints: 0, bools: false, strings: '', vectors: '0,0,0', textures_paths: '' };
    ...
    $(document).on('change', '#container-items-list .input-item-prop', function () {
        const index = $(this).data('index');
    ...
    $(document).on('change input', '#container-items-list .input-nested-prop', function () {
        const index = $(this).data('index');
    ...
    $(document).on('click', '#container-items-list .btn-remove-item', function () {
        const index = $(this).data('index');
    ...
    $('#form-container').on('submit', async function (e) {
        ...
        if (containerItems.length === 0) {
            Swal.fire('Atenção', 'Adicione pelo menos um item ao container.', 'warning');
            return;
        }
        ...
        for (let batchIndex = 0; batchIndex < maxVariations; batchIndex++) {
            ...
            const containerTitle = maxVariations > 1 ? `${baseTitle} #${batchIndex + 1}` : baseTitle;
            ...
            if (thumbSource === 'upload' && thumbFile) {
                ...
                formData.append('mainImage', capturedBlob, `canvas_${batchIndex + 1}.png`);
            }
                ...
                if (item.uploadFiles) {
                    Object.keys(item.uploadFiles).forEach(key => {
                        ...
                        if (Array.isArray(fileList) && fileList.length > 0) {
                            const currentFile = fileList[batchIndex] || fileList[fileList.length - 1];
                            if (currentFile) {
            ...
            try {
                const res = await fetch(URL_SAVE_CONTAINER, { method: 'POST', body: formData });
                ...
                Swal.fire('Erro', `Erro no lote #${batchIndex + 1}: ${err.message}`, 'error');
                return;
            }
        }

        Swal.fire('Sucesso!', `${maxVariations} container(s) gerado(s).`, 'success');
        $('#modal-container').addClass('hidden').removeClass('flex');
    });
});
...
function captureBabylonCanvas() {
    ...
    canvas.toBlob(blob => {
        ...
        $('#canvas-preview-img').attr('src', URL.createObjectURL(blob)).removeClass('hidden');
...
function renderModelGrid() {
    ...
    const allModels = (typeof MODELS !== 'undefined' ? MODELS : window.MODELS) || [];
    ...
    if (!filtered.length) {
        $grid.append('<div class="col-span-full text-center text-xs text-gray-500 py-4">Nenhum modelo encontrado</div>');
        return;
    }
...
function renderItemsList() {
    console.log("[SHOP] Renderizando lista de itens do container. Total de itens:", containerItems.length);
    const $list = $('#container-items-list').empty().addClass('max-h-[380px] overflow-y-auto pr-1');
...
function renderDynamicProperties(item, idx) {
        ...
        if (targetObj && Object.keys(targetObj).length > 0) {
            ...
            Object.keys(targetObj).forEach(key => {
                ...
                if (cat.type === 'number') {
                    ...
                    const labelText = count === 0 ? 'Nenhum arquivo selecionado' : (count === 1 ? filenames : `${count} arquivos: ${filenames}`);
                    const textColor = count > 0 ? 'text-green-400 font-bold' : 'text-gray-400';
    ...
    return html || '<span class="text-gray-500 text-[11px] italic">Nenhuma propriedade adicionada.</span>';
}
