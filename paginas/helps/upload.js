...
router.get("/helps/getfolders", (req, res) => {
...
router.get("/helps/downloadFile", (req, res) => {
  const filePath = req.query.path;
  if (!filePath) return res.status(400).send('Caminho não informado');
  res.download(path.resolve(filePath));
});

router.post("/helps/createFolder", express.json(), (req, res) => {
...
router.post("/helps/renameFile", express.json(), (req, res) => {
...
router.post("/helps/extractFile", express.json(), (req, res) => {
...
router.post("/helps/uploadFile", (req, res) => {
  let body = [];
  let destDir = '';
  let filesToSave = [];
...
router.get("/helps/downloadFolder", (req, res) => {
  const targetDir = req.query.path;
...
router.get("/helps/editFile", (req, res) => {
  const filePath = req.query.path;
  if (!filePath) return res.status(400).send('Caminho não informado');
...
router.post("/helps/saveFile", express.json(), (req, res) => {
...
router.post("/helps/deleteFile", express.json(), (req, res) => {
  const { dir, fileName } = req.body;
  if (!dir || !fileName) return res.status(400).send('Dados inválidos');
